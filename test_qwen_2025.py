...
for i in tqdm(range(len(data))):
    if "part3" in ckpt and f"dev/audio" not in data[i]['audios'][0]:
        continue
    if "part2" in ckpt and f"dev/fold" not in data[i]['audios'][0]:
        continue
    if "part1" in ckpt and (f"dev/audio" in data[i]['audios'][0] or f"dev/fold" in data[i]['audios'][0]):
        continue
    if i < len(exist_data):
        continue
    example = data[i]
