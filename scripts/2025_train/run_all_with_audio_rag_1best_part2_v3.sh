#!/bin/bash

#SBATCH --job-name=qwen2_5omni_2025_sharegpt_all-rag-with-audio
#SBATCH --output=2025_log/qwen2_5omni_2025_sharegpt_rag-with-audio-1best-part2-v3
#SBATCH --error=2025_log/qwen2_5omni_2025_sharegpt_rag-with-audio-1best-part2-v3
#SBATCH --ntasks=1
#SBATCH --time=08:00:00
#SBATCH --mem=200G
#SBATCH --nodes=1
#SBATCH --partition=p2,p3
#SBATCH --account=zhenwan.nlp
#SBATCH --gres=gpu:8
#SBATCH --cpus-per-task=8
#SBATCH --overcommit

source /mnt/home/zhenwan.nlp/miniconda3/etc/profile.d/conda.sh
conda activate factory

#default_model="/lustre/fsw/portfolios/nvr/users/hanrongy/checkpoints/xvila/xvila-nvila_8b-video_audBoostv2_s3_n8_bs2048_ga8_mstep-1_j20250414/model"
#default_output_dir="./eval_output"
#default_log_dir="./eval_log"
cd ~/LLaMA-Factory-Nbest
#llamafactory-cli train examples/train_lora/qwen2_5omni_lora_sft.yaml
FORCE_TORCHRUN=1 llamafactory-cli train examples/2025_train_full/qwen2_5omni_full_sft_2025_with_audio_rag_1best_part2.yaml
bash merge_full.sh "1best-part2"

conda deactivate
conda activate qwen_eval

CUDA_VISIBLE_DEVICES=0 python test_qwen_2025.py "1best-part2"
