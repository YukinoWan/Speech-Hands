...
task = sys.argv[1]
ckpt = sys.argv[2]
model_path = f"./target_dir/{task.split('_')[0]}-ger-with-audio-ckpt-{ckpt}"
...
if no_audio:
    with open(f"data/sharegpt_data/{task}_parakeet_5_best_no_audio.json", "r") as f:
        data = json.load(f)
else:
    with open(f"data/sharegpt_data/{task}_parakeet_5_best_with_audio.json", "r") as f:
        data = json.load(f)

output_dir = f"output/{task}_ger_with_audio_ckpt-{ckpt}" if not no_audio else f"output/{task}_ger_no_audio_ckpt-{ckpt}"
...
for i in tqdm(range(len(data))):
    ...
    #     print(f"Error: {example['conversations'][0]['value']}")
